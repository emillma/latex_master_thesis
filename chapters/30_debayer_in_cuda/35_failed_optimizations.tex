\subsection{Failed optimizations}
...
A minimal test was later created that performed a very simple repeated multiply and add operation, where the coefficient and constats were either stored in constant memory or defined as literal variables in the funciton as shown in \code{mfa_1} and \code{mfa_2} in Listing \ref{listing:cuda_mem_tests}.
...
Further it was tested weither storing the coefficient in constnt variables would improve performance as there is no built in \code{__half2} literal type, as shown in \code{mfa_3} in Listing \ref{listing:cuda_mem_tests}.
